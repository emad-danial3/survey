...
//dd($orders);
    ...
    public function userSurveys($id)
    {
        ...
        $usersMakeSurveyQuestions = DB::table('category_questions')
            ...
            ->select('users_surveys_details.question_id','category_questions.title',DB::raw("count(IF(users_surveys_details.chose_option !='option_5',1,null)) AS  total_count"),DB::raw("count(IF(users_surveys_details.chose_option='option_1',1,null)) AS  option_1_count"),DB::raw("count(IF(users_surveys_details.chose_option='option_2',1,null)) AS  option_2_count"),DB::raw("count(IF(users_surveys_details.chose_option='option_3',1,null)) AS  option_3_count"),DB::raw("count(IF(users_surveys_details.chose_option='option_5',1,null)) AS  option_5_count"))
            ->get();
        ...
        $usersMakeSurveyOptinalQuestions = DB::table('category_questions')
            ...
            ->get();
        $totalvoters=UsersSurveys::where('survey_id',$lastSurveyId)->count();
        ...
        for($i = 0;$i <count($usersMakeSurveyQuestions);$i++)
        {
           ...
           $usersMakeSurveyQuestions[$i]->total_option_5_percent=($lastSurvey->option_5_percent*$usersMakeSurveyQuestions[$i]->option_5_count);
//            dd($usersMakeSurveyQuestions[$i]->total_count);
           ...
           $usersMakeSurveyQuestions[$i]->total_percentage= round( $usersMakeSurveyQuestions[$i]->total_percentage, 2);
            ...
            $sum_option_5_count +=$usersMakeSurveyQuestions[$i]->option_5_count;
        ...
        return view('admin.users.surveys',compact('user','location','surveys','lastSurveyId','usersMakeSurveyQuestions','question_options','sum_option_1_count','sum_option_2_count','sum_option_3_count','sum_option_5_count','final_total_sum_percentage','usersMakeSurveyOptinalQuestions','totalvoters'));
    }
    ...
    public function getUserStatistic(Request $request)
    {
        ...
        for($i = 0;$i <count($usersMakeSurveyQuestions);$i++)
        {
            ...
            $usersMakeSurveyQuestions[$i]->total_percentage=(($usersMakeSurveyQuestions[$i]->total_option_1_percent +
            $usersMakeSurveyQuestions[$i]->total_option_2_percent+$usersMakeSurveyQuestions[$i]->total_option_3_percent)/($usersMakeSurveyQuestions[$i]->total_count));
